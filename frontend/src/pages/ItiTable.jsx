...
//     // Get the tradeIDs cookie value
//     const tradeIDs = getCookieValue('tradeIDs');
//     if (tradeIDs) {
//       const tradeIDArray = tradeIDs.split(',').map(Number); // Convert back to an array of numbers
//       console.log(tradeIDArray); // [2, 4], for example
//     }
...
function ItiTable() {
  ...
  useEffect(async () => {
    ...
    // Get the tradeIDs cookie value
    const tradeIDsString = getCookieValue('tradeIDs');
    if (tradeIDsString) {
      const tradeIDs = tradeIDsString.split(',').map(Number);
      console.log(tradeIDs); 
      try {
        const response = await fetch(`http://localhost:5000/iti/itis-by-trades?tradeIds=${tradeIDs}`, {
          method: 'GET',
          headers: {
  ...
  const filteredITIs = itis.filter(iti =>
    iti.institute_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    iti.city.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <table className="min-w-full bg-white mx-2">
          ...
          <tbody>
            {currentItems.map((iti, index) => (
              <tr
                key={iti.iti_id}
                onClick={() => handleRowClick(iti.iti_id)}
                className="cursor-pointer transition-all duration-200"
              >
                <td className="px-6 py-4 whitespace-nowrap">{index + 1}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.institute_name}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.address}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.taluka}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.city}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.contact_number}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.iti_code}</td>
                <td className="px-6 py-4 whitespace-nowrap">{iti.iti_type_id}</td>
              </tr>
            ))}
          </tbody>
        </table>
